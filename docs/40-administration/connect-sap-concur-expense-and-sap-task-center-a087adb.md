<!-- loioa087adb3150f4ca286bb8d70f5746d36 -->

# Connect SAP Concur Expense and SAP Task Center

Find information about the destination configuration that needs to be done for SAP Task Center in order to work with task from SAP Concur Expense on SAP BTP, Cloud Foundry environment.



<a name="loioa087adb3150f4ca286bb8d70f5746d36__prereq_zzy_spz_pjb"/>
...
2.  If you have performed the automatic setup \(see [Automatic Setup](https://help.sap.com/viewer/08cbda59b4954e93abb2ec85f1db399d/Prod/en-US/3a499676e7ae4282af84092f778e3737.html)\), you already have a sample destination called *Concur\_exp*. You can use the sample destination or clone it, and update the properties as described in the following table.
...
3.  Configure the properties of the destination as described next:
    ...
    *Client Key*
    ...
    This value is automatically replaced with a `refresh token` after a communication exchange between SAP Concur Expense and SAP Task Center.
...
4.  Select *New Property* on the right side of the *Destination Configuration* pane and add the following properties:
    ...
    *tc.concur.auth.req.token.enabled*
    ...
    This value is automatically changed to *false* after a communication exchange between SAP Concur Expense and SAP Task Center regarding the `Client Key`.

    > ### Caution:  
    > Changing the property manually after it was set to *false* by SAP Task Center can break the connection to the SAP Concur Expense destination.
    ...
    *tc.concur.dc*
    ...
    Corresponds to the data center, where this SAP Concur Expense destination is located \(see your `URL` parameter\).
    ...
    **Example for *tc.ui.label***:
    ...
    `SAP Concur Expense Task`

    **Example for *tc.ui.label.de-DE***:

    `SAP Concur Expense Aufgabe`
...
7.  \(Optional\) To check the connectivity between the SAP Task Center service and SAP Concur Expense, use the monitoring functionality of SAP Task Center. For more information, see [Monitoring](https://help.sap.com/viewer/08cbda59b4954e93abb2ec85f1db399d/Prod/en-US/9b30be76f14f48c5a72dd7ed7e9babe0.html).

    If you choose *Check Connection* in the destination configuration, you may not receive correct information about the connectivity between the SAP Task Center service and SAP Concur Expense.
